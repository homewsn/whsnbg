...
tcp_start:
	...
	{
		printf("MQTT: the TCP connection with whsnbg is established.\n");
	}
	for (;;)
	{
		fd_set rd;
		...
		if (res == 0)
		{
			...
			{
				if (send(sock, ms->msg_buf, (int)ms->msg_cnt, 0))
					print_error_socket(__LINE__);
		...
		if (FD_ISSET(sock, &rd))
		{
			...
			if (thread_state == THREAD_STAYING)
				break;
			recv_cnt = recv(sock, sock_buf, sizeof(sock_buf), 0);
			if (recv_cnt == 0)
			{
				printf("MQTT: whsnbg has closed the TCP connection.\n");
			...
			if (recv_cnt > 0)
				msg_tcp_mqtt_add_packet(&addr, sock_buf, recv_cnt);
			if (recv_cnt < 0)
				print_error_socket(__LINE__);
		}
	}
