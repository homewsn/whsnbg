...
static void thread_run(void *param)
{
	...
	for (cnt = 0; (cnt < test_mqttsn_list_get_size()) && (!test_failed); cnt++)
	{
		test = test_mqttsn_list_get_test(cnt);
		if (test->delay)
		{
			printf("MQTTSN: delay %d sec before sending the next command.\n\n", test->delay);
			tick_counter_set_mqttsn_mode(1);
			...
			tick_counter_set_mqttsn_mode(0);
		}
		if (test->command)
		{
		...
		for (;;)
		{
			...
			if ((ms = msg_udp_mqtt_get_first()) != NULL)
			{
	...
	if (test_failed)
	{
		thread_test_mqtt_cancel();
		printf("MQTTSN tests are failed.\n");
	}
	else
	{
		printf("MQTTSN tests passed successfully.\n");
	}
	thread_test_mqtt_exitmsg();
	printf("--------------------------------------------\n\n");
...
test_cancel:
	...
	{
		tick_counter_set_mqttsn_mode(0);
		printf("MQTTSN tests are cancelled.\n");
	}
	if (exitmsg_flag)
	{
...
//--------------------------------------------
void thread_test_mqttsn_start(void)
{
	pthread_t thread;
	void *param = NULL;
...
//--------------------------------------------
void thread_test_mqttsn_stop(void)
{
...
//--------------------------------------------
void thread_test_mqttsn_cancel(void)
{
	cancel_flag = 1;
}

//--------------------------------------------
void thread_test_mqttsn_exitmsg(void)
{
	exitmsg_flag = 1;
}
